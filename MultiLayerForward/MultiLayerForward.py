...
def calWhiteResult(strategyName, whiteWindows, symbolinfo, K_MIN, parasetlist, monthlist, datapath, resultpath, columns,
    ...
    for i in range(parasetlen):
        ...
        dailydf = pd.read_csv(
            datapath + strategyName + ' ' + symbol + str(K_MIN) + ' ' + setname + ' daily' + filesuffix)
...
def rankByWhiteResult(strategyName, symbolinfo, K_MIN, whiteWindows, datapath, resultpath):
    ...
    for month in col:
        df = pd.DataFrame({'Setname': annualdf.Setname, 'annual': annualdf[month], 'sharpe': sharpedf[month],
                           'success_rate': successdf[month], 'drawback': drawbackdf[month]})
        df['AnnualRank'] = 0
        df['SharpeRank'] = 0
        df['SuccessRank'] = 0
        df['DrawbackRank'] = 0
        ...
        df['Rank5'] = df['AnnualRank'] * 0.4 + df['SharpeRank'] * 0.3 + df['SuccessRank'] * 0.1 + df[
                                                                                                      'DrawbackRank'] * 0.2  # 目标集5：4目标综合
...
def calGrayResult(strategyName, symbol, K_MIN, windowsSet, rankpath, rawdatapath):
    ...
    setresultdf.to_csv(rawdatapath + 'ForwardOprAnalyze\\' + strategyName + ' ' + symbol + str(
        K_MIN) + 'multiTargetForwardSetname.csv')
    pass


def getOprlistByMonth(strategyName, rawpath, symbol, K_MIN, setname, startmonth, endmonth, columns,
                      filesuffix='result.csv'):
    '''
    ...
    return oprdf[
        ['opentime', 'openutc', 'openindex', 'openprice', closetime_col, closeutc_col, closeindex_col, closeprice_col,
         'tradetype', ret_col, retr_col, 'symbol']]


def calOprResult(strategyName, rawpath, symbolinfo, K_MIN, nextmonth, columns, barxmdic, positionRatio, initialCash,
                 indexcols, indexcolsFlag, resultfilesuffix='result.csv'):
    '''
    ...
    symbol = symbolinfo.domain_symbol
    graydf = pd.read_csv(
        rawpath + 'ForwardOprAnalyze\\' + strategyName + ' ' + symbol + str(K_MIN) + 'multiTargetForwardSetname.csv',
        index_col='Group')
    ...
    for i in range(graydf.shape[0]):
        gray = graydf.iloc[i]
        oprdf = pd.DataFrame(
            columns=['opentime', 'openutc', 'openindex', 'openprice', closetime_col, closeutc_col, closeindex_col,
        ...
        for l in range(len(cols) - 1):
            ...
            oprdf = pd.concat([oprdf,
                               getOprlistByMonth(strategyName, rawpath, symbol, K_MIN, setname, startmonth, endmonth,
                                                 columns, resultfilesuffix)])
    ...
    groupResultDf.to_csv(
        rawpath + 'ForwardOprAnalyze\\' + strategyName + ' ' + symbol + '_' + str(K_MIN) + '_groupOprResult.csv',
        index=False)
    pass
...
def runPara(strategyName, whiteWindows, symbolinfo, K_MIN, parasetlist, monthlist, rawdatapath, resultpath, rankpath,
            columns, filesuffix):
    calWhiteResult(strategyName=strategyName, whiteWindows=whiteWindows, symbolinfo=symbolinfo, K_MIN=K_MIN,
                   parasetlist=parasetlist,
                   monthlist=monthlist, datapath=rawdatapath, resultpath=resultpath, columns=columns,
                   filesuffix=filesuffix)
    rankByWhiteResult(strategyName=strategyName, symbolinfo=symbolinfo, K_MIN=K_MIN, whiteWindows=whiteWindows,
